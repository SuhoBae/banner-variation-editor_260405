...
[2026-04-07] Image Panel Buttonization and Saved-History Row Layout

Situation: The text-layer right-side alignment/geometry controls had become visually noisy relative to their daily value, and the image panel still exposed image upload mainly as a dropzone instead of explicit actions. The saved-history tray also rendered thumbnails in a card layout that did not preserve the intended square emphasis and made scanability weaker.

Resolution: Removed the text-layer X/Y/W/H numeric block and kept the text side panel focused on content editing, typography, alignment, and board-scoped state actions. Reworked the image panel so it always exposes explicit `Upload PNG` and `Generate image` buttons under the PNG area, while preserving drag-and-drop upload. Added a generation entry point that currently explains the missing backend requirement instead of pretending the feature is live. Rebuilt the saved-history entries into horizontal rows with the information and action buttons on the left and a dedicated square thumbnail area on the right so saved states read more cleanly.

[2026-04-07] OpenAI Image Generation Endpoint Added

Situation: The editor now had a visible `Generate image` button but still lacked any real image-generation backend, so the action could not progress beyond a placeholder explanation. The project also needed a safe way to keep `OPENAI_API_KEY` off the client while still supporting Vercel production and local development.

Resolution: Added a shared server-side image generation module at `server/openai-image.js`, a Vercel function at `api/generate-image.js`, and a Vite dev middleware route in `vite.config.js` so `/api/generate-image` works in both production and local dev. The frontend now asks for a prompt, posts the current board size to the server endpoint, maps the board ratio to a supported OpenAI image size, and applies the returned PNG data URL directly to the existing image layer. Added `.env.example` and `.gitignore` rules for `OPENAI_API_KEY` and `OPENAI_IMAGE_MODEL`. Current scope is full-image generation only; partial region generation still needs a mask-selection UI and an edit/inpainting endpoint.

[2026-04-07] Image-AI Rollback and Upload-Only Simplification

Situation: The project briefly added an OpenAI-backed `Generate image` flow, but the user chose the cheaper and simpler product direction: remove paid image generation entirely and keep the editor centered on manual PNG uploads. Leaving the AI path half-wired would have kept extra UI, server code, env handling, and docs that no longer matched the actual product.

Resolution: Removed the `Generate image` button, the OpenAI/Vercel image-generation endpoint, the dev middleware, and the related environment-file example so the image panel now has a single upload-focused interaction model. Reused the existing image application helper for local PNG uploads instead of keeping duplicated code paths. When product direction changes, prefer deleting the abandoned path end-to-end rather than hiding it behind a broken or optional button.

[2026-04-08] Multi-Artboard Selection and Scoped Apply Model
